// ============================================================================
// GAZ TIME - Database Schema (Drizzle ORM) - WITH FK CONSTRAINTS
...
export const customers = pgTable('customers', {
  ...
  referred_by: text('referred_by').references(() => customers.id, { onDelete: 'set null' }),
  ...
  referredByIdx: index('customers_referred_by_idx').on(table.referred_by),
}));
...
export const orders = pgTable('orders', {
  ...
  pod_id: text('pod_id').references(() => pods.id, { onDelete: 'set null' }),
  ...
  podIdx: index('orders_pod_idx').on(table.pod_id),
...
export const drivers = pgTable('drivers', {
  ...
  userIdx: index('drivers_user_idx').on(table.user_id),
...
export const pods = pgTable('pods', {
  ...
  operator_id: text('operator_id').references(() => users.id, { onDelete: 'set null' }),
  ...
  operatorIdx: index('pods_operator_idx').on(table.operator_id),
}));

export const podsRelations = relations(pods, ({ many, one }) => ({
  operator: one(users, {
    fields: [pods.operator_id],
    references: [users.id],
  }),
  orders: many(orders),
}));
...
export const wallets = pgTable('wallets', {
  id: text('id').primaryKey(),
  customer_id: text('customer_id').notNull().unique().references(() => customers.id, { onDelete: 'cascade' }),
...
export const walletTransactions = pgTable('wallet_transactions', {
  id: text('id').primaryKey(),
  wallet_id: text('wallet_id').notNull().references(() => wallets.id, { onDelete: 'cascade' }),
  type: text('type').notNull(),
...
export const subscriptions = pgTable('subscriptions', {
  ...
  product_id: text('product_id').notNull().references(() => products.id, { onDelete: 'restrict' }),
  ...
  productIdx: index('subscriptions_product_idx').on(table.product_id),
  statusIdx: index('subscriptions_status_idx').on(table.status),
  nextDeliveryIdx: index('subscriptions_next_delivery_idx').on(table.next_delivery_date),
}));
