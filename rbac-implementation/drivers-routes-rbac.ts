// ============================================================================
// Driver Routes - WITH RBAC IMPLEMENTATION
...
import { authenticate, requireRole, requireResourceAccess, type AuthUser } from '../middleware/auth.js';
import type {
  DriverStatus,
...
export async function driverRoutes(fastify: FastifyInstance) {
  ...
  // ✅ RBAC: admin, operator can update any driver; drivers can only update their own status
  fastify.patch<{ Params: { id: string }; Body: { status: DriverStatus } }>(
    '/drivers/:id/status',
    {
      onRequest: [requireResourceAccess('driver')], // ✅ CHANGED: Added ownership check
    },
    async (request: any, reply) => {
      try {
  ...
  // ✅ RBAC: admin, operator can update any driver; drivers can only update their own location
  ...
  // Complete delivery
  // ✅ RBAC: admin, operator can complete any delivery; drivers can only complete their assigned deliveries
  fastify.post<{ Body: { orderId: string; proof: DeliveryProof } }>(
    '/drivers/complete-delivery',
    {
      onRequest: [authenticate], // Keep authenticate, add check inside handler
    },
    async (request: any, reply) => {
      try {
        ...
        if (user.role === 'driver') {
          ...
          const [order] = await db.select().from(orders).where(eq(orders.id, request.body.orderId));
          ...
          if (order.driver_id !== user.id) {
            return reply.code(403).send({
              ...
              error: {
                code: 'FORBIDDEN',
                message: 'Drivers can only complete their own assigned deliveries',
              },
            });
          }
        }
